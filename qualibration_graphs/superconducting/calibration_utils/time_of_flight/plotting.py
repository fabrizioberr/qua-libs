...
def plot_individual_single_run_with_fit(ax: Axes, ds: xr.Dataset, qubit: dict[str, str], fit: xr.Dataset = None):
    ...
    """
    fit.adc_single_runI.plot(ax=ax, x="readout_time", label="I", color="b")
    fit.adc_single_runQ.plot(ax=ax, x="readout_time", label="Q", color="r")
    ax.axvline(fit.delay, color="k", linestyle="--", label="TOF")
    ax.axhline(fit.offsets_I_mean, color="b", linestyle="--")
    ax.axhline(fit.offsets_Q_mean, color="r", linestyle="--")
    ax.fill_between(
        range(fit.sizes["readout_time"]),
        -0.5,
        0.5,
        color="grey",
        alpha=0.2,
...
def plot_individual_averaged_run_with_fit(ax: Axes, ds: xr.Dataset, qubit: dict[str, str], fit: xr.Dataset = None):
    ...
    - If the fit dataset is provided, the fitted curve is plotted along with the raw data.
    """
    fit.adcI.plot(ax=ax, x="readout_time", label="I", color="b")
    fit.adcQ.plot(ax=ax, x="readout_time", label="Q", color="r")
    ax.axvline(fit.delay, color="k", linestyle="--", label="TOF")
    ax.axhline(fit.offsets_I, color="b", linestyle="--")
    ax.axhline(fit.offsets_Q, color="r", linestyle="--")
    ax.set_xlabel("Time [ns]")
    ax.set_ylabel("Readout amplitude [V]")
    ax.set_title(qubit["qubit"])
